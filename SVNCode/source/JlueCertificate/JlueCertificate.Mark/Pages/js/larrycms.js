...
layui.use(['jquery', 'larryElem', 'layer', 'common', 'form', 'larryMenu', 'larryTab'], function () {
    ...
    Params.Ajax("/Handler/UserCenter.ashx?action=userinfo", "get", "", refreshUserInof_success, refreshUserInof_fail);
    function refreshUserInof_success(ret) {
        if (ret.Code == "0") {
            var _url = 'datas/data9.json?t=' + Math.random()
            if (ret.Data != null && ret.Data.Level != "9") {
                _url = 'datas/data1.json?t=' + Math.random()
            }
            $.getJSON(_url, function (menuData) {
                larryTab.menuSet({
                    data: menuData,
                    spreadOne: false,
                    topFilter: 'TopMenu',
    ...
    function refreshUserInof_fail(ret) {
        layer.msg(ret.Msg);
    }
    ...
    //larryTab.menuSet({
    //    tyep: 'GET',
    //    url: 'datas/data9.json?t=' + Math.random(),
    //    topFilter: 'TopMenu',
    //    lefFilter: 'LarrySide'
    //});
    //larryTab.menu();
