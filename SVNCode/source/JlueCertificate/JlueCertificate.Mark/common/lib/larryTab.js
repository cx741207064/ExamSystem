...
layui.define(['jquery', 'larryElem', 'layer', 'common'], function(exports) {
    ...
    if (_config.data !== undefined && typeof(_config.data) === 'object') {
      ...
      $.ajax({
        ...
        error: function(xhr, status, error) {
          common.larryCmsError('Error: ' + error, common.larryCore.errorDataTit);
        },
        complete: function() {
          _that.init();
        }
  ...
  LarryTab.prototype.tabAdd = function(data) {
    ...
    if (tabIndex == -1) {
      ...
      if (_config.closed) {
          title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + gobalTabIndex + '"></i>';
  ...
  LarryTab.prototype.recoveryTab = function(tab) {
        ...
        if (tabIndex == -1) {
            ...
            if (_config.closed) {
                title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + tab.id + '"></i>';
              ...
              for (var j = 0; j < data[i].children.length; j++) {
                  if (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0) {
                      for (var k = 0; k < data[i].children[j].children.length; k++) {
                          if (data[i].children[j].children[k].url !== undefined && data[i].children[j].children[k].url !== null && data[i].children[j].children[k].url.length > 0) {
                              index++;
                              var current1 = {
                                  icon: data[i].children[j].children[k].icon,
                                  title: data[i].children[j].children[k].title,
                                  href: data[i].children[j].children[k].url,
                                  id: index,
                                  group: 0,
                                  closed: false
                              };
                              tabMenu1.push(current1);
                          }
                      }
                  }
                  else {
                      ...
                      var current2 = {
                            ...
                            href: data[i].children[j].url,
                            id: index,
                            group: 0,
                            closed: false
                      };
                      tabMenu1.push(current2);
                  }
              }
          }
      }
      sessionStorage.setItem('tabMenu', JSON.stringify(tabMenu1));
  }
  /**
   * www.larrycms.com
